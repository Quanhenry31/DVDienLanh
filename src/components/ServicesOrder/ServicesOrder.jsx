...
function ServicesOrderComp() {
    ...
    const handleChange = (event) => {
        ...
        const selectedCategory = searchName.rows
            .flatMap((service) => service.ServiceCategories)
    ...
    useEffect(() => {
        axios
            .get('http://localhost:9000/api/servicesOrder/all')
            .then((response) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`http://localhost:9000/api/servicesOrder`, {
                    params: {
                ...
                setSelectedPriceDefau(response?.data?.rows[0]?.ServiceCategories[0]?.price);
                setSelectedCapacity(response?.data?.rows[0]?.ServiceCategories[0]?.capacity);
                setSelectedTitle(response?.data?.rows[0]?.ServiceCategories[0]?.title);
                dispatch(addToConten(response?.data?.rows[0]?.ServiceCategories[0]?.title));
            } catch (error) {
                console.error('There was an error fetching the product data!', error);
            }
        };
    ...
    const handleServicesOrderPay = (event) => {
        ...
        const sentValues = {
            ...
            typeServices: 'Dịch vụ đặt lịch bảo dưỡng',
        };
        console.log(sentValues);

        if (user.type) {
            axios
                .post(`http://localhost:9000/api/servicesOrderPays`, sentValues)
                .then((response) => {
                    ...
                    <div className="col-lg-5 offset-lg-1">
                        <form onSubmit={handleServicesOrderPay}>
                            <div className="s_product_text">
                                <h3>Dịch vụ đặt lịch bảo dưỡng</h3>
...
export default ServicesOrderComp;
